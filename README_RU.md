...
mn.select("g1.r7", returns="values").first()      # select() может кончаться на селекторе строки: "g1.r7" = сама строка, "g1.?" = все строки, "g1" = запись, "?" = все записи
...
`items`, `to_dict`, `copy`, `serialize`, `len()`, итерация, `.at(i)`),
...
Вся эта raw-группа читает/пишет привязанную таблицу напрямую и **не видит
`set_sort()`/`set_filter()`** — `keys()` вернёт все строки в собственном
порядке таблицы даже при активном фильтре. Учитывающие sort/filter аналоги —
`view_keys()`/`view_values()`/`view_items()` (ниже); разделение сделано по
имени, чтобы ни один из смыслов не подразумевался молча. `copy()`/
`serialize()` работают с привязанной *таблицей* и дают самостоятельный
корневой `ModDict`/его байты — sort/filter/connect курсора это представление,
а не данные, и в payload не попадает.

**Что курсор не поддерживает:** сугубо root-овые операции — `link`/`follow`
(связи объявляются на root-путях таблиц, курсор живёт *внутри* одной
таблицы), `reindex` (неоднозначно: ключ относительно root'а или anchor'а?),
`deserialize` (заменяет содержимое целиком). Отдельно:
`filter`/`sort_by`/`group_by`/`select`/`select_mass`/`update` и
`create_index`/`drop_index`/`has_index` кидают потому, что читают собственное
хранилище root'а, которого у курсора нет — зовите их на root'е. Это
одноразовые запросы, возвращающие данные, и они не конкурируют с
...
Вся raw-группа (`cursor[key]`, `in`, `del`, `get`, `pop`, `keys`/`values`/
`items`, `to_dict`, ...) от фильтра не зависит в любом случае —
отфильтрованная строка всё ещё полностью присутствует в данных, просто
отсутствует в позиционном/итерационном виде.
...
# raw-группа о фильтре/сортировке ничего не знает: отфильтрованная строка
# всё равно достижима напрямую:
orders["o2"]["amount"]             # работает, даже если "o2" не проходит активный фильтр
orders.get("o2")                   # то же самое, без исключения
"o2" in orders                     # True вне зависимости от фильтра
orders.keys()                      # ВСЕ ключи, в собственном порядке таблицы
"o2" in orders.view_keys()         # False — исключена из вида
```

`view_keys()`/`view_values()`/`view_items()` названы отдельно от
`keys()`/`values()`/`items()` намеренно: raw-группа на курсоре означает ровно
то же, что и на обычном словаре (фильтр/сортировку не видит) — значит
...
```python
md.set_geo_backend("shapely")       # -> shapely-геометрия
md.set_geo_backend("geoalchemy2")   # -> WKBElement
md.set_geo_backend("wkb_bytes")     # -> сырые байты WKB, без разбора
md.set_geo_backend(None)            # сбросить -> автоопределение (ниже)
```

`"wkb_bytes"` не требует ни одной установленной библиотеки и полностью
убирает стоимость разбора/аллокации на каждое значение — правильный выбор,
когда геометрия только транзитом едет дальше (в другой сервис, обратно в
базу) и геометрически не используется. Заодно результат перестаёт зависеть
от того, что оказалось `pip install`-ано в конкретном окружении.

Без явного выбора восстановление зависит от стороны чтения:

- Установлена только одна из двух библиотек → используется автоматически,
  вызов не нужен.
- Установлены обе → **обязателен вызов** — десериализация геометрии без
  предварительного вызова кидает `ValueError`. Это намеренно ошибка, а не
  догадка: выбор определяет возвращаемый *тип*, поэтому угадывание было бы
  источником тихих неверных результатов.
- Не установлена ни одна → возвращаются сырые байты WKB, данные не
  теряются. Но если тебе нужны именно байты — лучше сказать `"wkb_bytes"`
  явно, а не полагаться на отсутствие библиотеки.
