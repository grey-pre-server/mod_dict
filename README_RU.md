...
mn.select(["orders.?.customer_id->name"])                # wildcard/"->"-поля тоже — {order_pk: {"name": ...}}
...
падают на скан таблицы-якоря. `returns="rows_here"`/`"values"` с `->`-путём
не поддерживаются (семантика неоднозначна, когда задействованы две таблицы).
