...
new_index = orders.insert("o9", {"amount": 15})  # → int | None — позиция, куда попала строка
...
orders.insert(key, row)          # -> int | None (новая позиция)
...
# → int | None — позиция ИМЕННО этой строки, а не список всех сдвинутых
#   соседей. Подавай прямо в Qt-шный beginInsertRows(pos, pos) — сам Qt уже
#   знает, что всё после pos сдвинулось, явный список не нужен. None —
#   значит активный фильтр строку не показывает (записана, но не видна).
...
# так же, как неявный сдвиг при insert/remove. changed_fields: только те
# поля, значение которых реально изменилось.
...
# → list[int | None], по одному на новую строку, в порядке батча — одна
